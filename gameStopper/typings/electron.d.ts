...
You should call `event.preventDefault()` if you want to handle this event.
    ...
    removeListener(
      event: "ready",
      listener: (launchInfo: unknown) => void
    ): this;
    /**
...
Calculate system idle time in seconds.
  ...
  interface Protocol {
    ...
    interceptBufferProtocol(
      scheme: string,
      handler: (request: Request, callback: (buffer?: Buffer) => void) => void,
    ...
    interceptFileProtocol(
      scheme: string,
      handler: (request: Request, callback: (filePath: string) => void) => void,
      completion?: (error: Error) => void
    ): void;
    /**
     * Intercepts `scheme` protocol and uses `handler` as the protocol's new handler
    ...
    interceptStreamProtocol(
      ...
      handler: (
        request: Request,
        callback: (
          stream?: NodeJS.ReadableStream | StreamProtocolResponse
        ) => void
      ) => void,
      completion?: (error: Error) => void
    ): void;
    ...
    interceptStringProtocol(
      ...
      handler: (
        request: Request,
        callback: (data?: string | StringProtocolResponse) => void
      ) => void,
      completion?: (error: Error) => void
    ): void;
    /**
...
Example:
    ...
    registerFileProtocol(
      ...
      handler: (
        request: Request,
        callback: (filePath?: string | FilePathWithHeaders) => void
      ) => void,
      completion?: (error: Error) => void
    ): void;
    /**
...
The usage is the same with the `login` event of `app`.
    ...
    on(
      event: "remote-get-guest-web-contents",
      listener: (event: IpcMainEvent, guestWebContents: WebContents) => void
    ): this;
    once(
      event: "remote-get-guest-web-contents",
      listener: (event: IpcMainEvent, guestWebContents: WebContents) => void
